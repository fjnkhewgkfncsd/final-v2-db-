...
const Dashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      // Fetch system status for admin/staff only
      if (isAdmin || isStaff) {
      ...
      // Fetch quick actions data for all authenticated users
      promises.push(axios.get('/api/analytics/dashboard', { headers }));

      const results = await Promise.all(promises);
      
      let resultIndex = 0;
      
      // Process user stats (only if admin/staff)
      if (isAdmin || isStaff) {
      ...
      // Process system status (only if admin/staff)
      if (isAdmin || isStaff) {
      ...
      // Process performance data (only if admin/staff)
      if (isAdmin || isStaff) {
      ...
      // Process quick actions data (for all users)
      const quickActionsResponse = results[resultIndex++];
      if (quickActionsResponse.data.success) {
        setQuickActions(quickActionsResponse.data.data.quickActions);
        console.log('✅ Quick actions loaded:', quickActionsResponse.data.data.quickActions);
      } else {
        console.error('❌ Failed to load quick actions:', quickActionsResponse.data);
      }

    } catch (error) {
      console.error('Error fetching dashboard data:', error);
      
      // Set some default quick actions data for testing
      setQuickActions({
        pendingOrders: 0,
        lowStockProducts: 0,
        recentNotifications: []
  ...
  const QuickActions = () => {
    console.log('QuickActions component rendering, quickActions state:', quickActions);
      ...
      <div className="card">
        ...
        {!quickActions && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            <p className="text-sm text-yellow-800">Loading quick actions data...</p>
          </div>
        )}
