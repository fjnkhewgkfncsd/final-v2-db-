...
// Dashboard analytics endpoint with role-based restrictions
router.get('/dashboard', auth, authorizeRoles(['admin', 'staff']), async (req, res) => {
    try {
        ...
        // Quick actions data for dashboard
...
// Get real-time system performance metrics - Admin only
router.get('/system-performance', auth, authorizeRoles(['admin']), async (req, res) => {
    try {
...
// Get system status information - Admin and Staff only  
router.get('/system-status', auth, authorizeRoles(['admin', 'staff']), async (req, res) => {
    try {
